...
SQL_FILE="${ROOT_DIR}/supabase/tests/rbac_regression.sql"

if ! command -v docker >/dev/null 2>&1; then
  echo "docker is required to run RBAC SQL regression checks." >&2
  exit 1
fi
...
echo "Running RBAC SQL regression checks against container '${CONTAINER_NAME}'..."
