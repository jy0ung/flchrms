-- =============================================================================
-- Migration: admin_create_employee validation fix
-- Purpose:   Fix runtime validation errors in admin_create_employee caused by
--            schema-qualifying COALESCE (which is not callable as a function).
-- =============================================================================
...
BEGIN
  ...
  -- ── Input validation ───────────────────────────────────────────────────
  IF pg_catalog.trim(coalesce(_email, '')) = '' THEN
  ...
  IF pg_catalog.trim(coalesce(_first_name, '')) = '' THEN
  ...
  INSERT INTO auth.users (
    ...
    jsonb_build_object(
      ...
      'last_name', pg_catalog.trim(coalesce(_last_name, '')),
      'email_verified', true,
