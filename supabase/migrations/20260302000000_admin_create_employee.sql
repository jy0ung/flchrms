-- =============================================================================
-- Migration: admin_create_employee RPC
-- Purpose:   Allow HR and Director roles to create new employee accounts via a
--            SECURITY DEFINER function that inserts into auth.users and lets the
--            existing handle_new_user trigger auto-create the profiles row and
--            default employee role. Optional profile fields are then patched.
-- =============================================================================
...
BEGIN
  ...
  -- Only HR and Director may create employees (matches profiles INSERT RLS)
  IF NOT (
    public.has_role(requester_id, 'hr'::public.app_role)
    OR public.has_role(requester_id, 'director'::public.app_role)
  ) THEN
    RAISE EXCEPTION 'Insufficient privileges: only HR or Director may create employees'
      USING ERRCODE = '42501';
  END IF;
...
-- Grant execute to authenticated users (RPC role gate handles authorization)
...
COMMENT ON FUNCTION public.admin_create_employee IS
  'Creates a new employee auth account and profile. HR/Director only. '
