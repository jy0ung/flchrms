-- Migration: Server-side leave approval RPC
-- Fixes TOCTOU race condition by performing status check + update atomically
-- within a single transaction with row-level locking.

-- ============================================================
-- approve_leave_request: Atomic leave approval/rejection
-- ============================================================
...
BEGIN
  ...
  IF _next_stage = 'manager' THEN
    UPDATE leave_requests SET
      ...
      final_approved_by_role = CASE WHEN _is_final THEN 'manager' ELSE NULL END,
  ...
  ELSIF _next_stage = 'general_manager' THEN
    UPDATE leave_requests SET
      ...
      final_approved_at = CASE WHEN _is_final THEN _now ELSE NULL END,
      final_approved_by = CASE WHEN _is_final THEN _approver_id ELSE NULL END,
      final_approved_by_role = CASE WHEN _is_final THEN 'general_manager' ELSE NULL END,
      hr_notified_at = CASE WHEN _is_final THEN _now ELSE NULL END,
      updated_at = _now
    WHERE id = _request_id;
  ...
  ELSIF _next_stage = 'director' THEN
    UPDATE leave_requests SET
      ...
      final_approved_at = CASE WHEN _is_final THEN _now ELSE NULL END,
      final_approved_by = CASE WHEN _is_final THEN _approver_id ELSE NULL END,
      final_approved_by_role = CASE WHEN _is_final THEN 'director' ELSE NULL END,
      hr_notified_at = CASE WHEN _is_final THEN _now ELSE NULL END,
      updated_at = _now
    WHERE id = _request_id;
  END IF;
